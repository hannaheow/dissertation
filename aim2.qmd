...
All models were fit using the *splm* and *splines* R packages [@splines-2; @splm-3]. Splines ranged from two to five degrees of freedom. Models were compared using the Bayesian Information Criterion (BIC), which is commonly used to identify models with the greatest explanatory power [@kuha2004]. Models were selected for subsequent analyses based on having the lowest BIC score or being within approximately 10 points of the minimum BIC score [@multimod], while also ensuring numerical stability.
...
# Add caption. Combine tables. Remove “total” column and remove “missing” row. N’s don’t change so just put the one value next to “rural counties (n=xxx)” and “urban counties (n=xxx)” Focus on either mean/SD or median/IQR/range, likely the former. (And eventually format).
...
# add below: Note: migration rates for the year 2011 are not included. This is because the migration-flow data starts from 2011 to 2012, and these values are shown in the 2012 section of the table, corresponding with the 2012 premature age-adjusted mortality rates.
