...
talk about stability / consistency (discussion section)

check the citations

check for the word CITE

maps of 4, 5, 6 components in an appendix????
...
~~Thus, migration plays an essential role in shaping place-based health disparities, with both origin and destination counties affected by population movement. Structural differences between regions—such as those between urban and rural areas or between affluent and economically disadvantaged counties—interact with migration patterns to either mitigate or worsen health inequalities. Counties with weaker infrastructure, fewer health services, and lower economic stability may experience further declines in health as younger residents migrate away. In contrast, destination counties may benefit economically and socially from the influx of migrants, but these health benefits are not always evenly distributed and can add strain on local services if growth is too rapid. Migration, then, is deeply intertwined with social determinants of health, and its analysis requires a multidimensional approach that can capture its effects across regions over time.~~
...
-   $r$ is the number of components, selected based on the elbow plot
...
~~To evaluate the fit of our models, we conducted a comparative analysis using the Akaike Information Criterion (AIC). AIC provides a measure of model quality that balances model fit and complexity, allowing us to assess which model best explains the data with the least amount of error. AIC is particularly useful when the true model is known to be complex [@vrieze2012], as is expected for any model of place-based health. We compared four models with varying configurations: some included coefficients for urbanicity, while others did not, allowing us to explore the impact of urbanicity adjustments on model performance. Additionally, we assessed models that included coefficients for all three tensor decomposition components versus those restricted to only the first (and most prominent) component. By examining AIC scores across these different models, we could determine whether adding urbanicity and/or additional components significantly improved model fit or simply introduced unnecessary complexity. The model with the lowest AIC score was deemed the most efficient, as it achieved the best balance between fit and simplicity, guiding our understanding of the importance of urbanicity and the relevance of multiple components in explaining migration patterns.~~
...
create_side_by_side_map <- function(component_num) {
 ...
 # Combine the two maps with a shared title and single legend
  combined_plot <- cowplot::plot_grid(
    p_orig, p_dest, ncol = 2, align = "v", rel_widths = c(1,1)
  )
...
It’s important to note that our analysis cannot fully capture the reasons behind these address changes or definitively explain why individuals might have relocated. Multiple factors, including personal, social, and economic reasons, can influence migration patterns, and these motivations remain beyond the reach of our current dataset. Consequently, while we observe notable trends for 2013 and 2018, attributing these patterns solely to policy shifts involves a degree of speculation, as the true drivers of migration decisions are inherently complex and multifaceted.

##### Top Origins and Destinations in Component 1 
...
In examining the top origin and destination counties within Component 2, Genesee County, Michigan—the site of the 2014 Flint Water Crisis—stands out as the primary origin. While we do not analyze specific migration motivations in this study, we suspect that Component 2 might capture out-migration linked to the crisis. Although survey research has not conclusively demonstrated that the Flint Water Crisis led to substantial out-migration, many residents reported considering relocation in response to it [@morckel2018]. However, the time dimension of Component 2 does not fully support this hypothesis, as the years 2014 and 2015 were not particularly influential.
...
In the maps for Component 3, we again observe a broader distribution of ranked loadings among destination counties than among origin counties. Highly ranked origin counties are scattered across the U.S., whereas highly ranked destination counties appear more evenly distributed and clustered. Additionally, the years 2013 and 2018 play a prominent role in explaining the migration patterns captured by Component 3 while years 2019 and 2017 are the least influential.

##### Top Origins and Destinations in Component 3
...
-   $\beta_{o_n}$ ~~are the coefficients for origin loadings~~ $o_1$~~,~~ $o_2$~~, and~~ $o_3$

-   $\beta_{d_n}$ ~~are the coefficients for destination loadings~~ $d_1$~~,~~ $d_2$~~, and~~ $d_3$

-   $\beta_{t_n}$ ~~are the coefficients for time loadings~~ $t_1$~~,~~ $t_2$~~, and~~ $t_3$

-   $\gamma_i$ ~~are the coefficients for urbanicity categories where the most urban category (large central metro) is the reference~~ <!--# not sure if i should expand here or not .... \gamma_{\text{LFM}} \text{Large Fringe Metro}_i + \gamma_{\text{MM}} \text{Medium Metro}_i + \gamma_{\text{SM}} \text{Small Metro}_i + \gamma_{\text{MIC}} \text{Micropolitan}_i + \gamma_{\text{R}} \text{Rural}_i + u_{it}  -->

-   $\mu_{it}$ ~~represents the spatial error term accounting for random effects in the model with spatial autoregressive errors.~~

The spatial panel model revealed several significant findings, as shown in the table below. The origin and destination loadings capture variability in health outcomes based on the origin and destination characteristics of migration flows. Notably, the coefficients for the second origin loading $\beta_{o_2}$ and the first and third destination loadings $\beta_{d_1}$ and $\beta_{d_3}$ are statistically significant. Increased $o_2$ (ie increased loadings in Component 2 - *Specific Phenomena*) are associated with higher premature mortality rates increased $d_1$ and $d_3$ (ie loadings in Component 1 - *Urban-to-Rural* and Component 3 - *State borders*) are associated with lower premature mortality rates. This may suggest that counties contributing greatly as an origin of migration is negative for county-level health while contributing greatly as a destination of migration is positive for county-level health.
...
all the different transformations i tested; pop adj ; climate change and inequality ; acs data
...
~~From a public health perspective, understanding each county's unique role within a migration system can help policymakers create more tailored interventions. Some regions may be experiencing population loss and the resulting strain on healthcare systems and social services. In contrast, other regions may demonstrate more stable or protective migration patterns, and may offer lessons in fostering environments that mitigate the negative health impacts of migration. This emphasizes the importance of understanding migration as a dynamic process that shapes health outcomes in both origin and destination areas.~~

~~The findings of this study are consistent with previous research linking migration and health outcomes but add new insights through the use of tensor decomposition to capture the complex nature of migration flows. Unlike traditional methods that focus on static or linear migration patterns, tensor decomposition allows for the identification of multifaceted migration dynamics and their spatial and temporal evolution. This approach provides a more nuanced understanding of how migration influences health, revealing both risks and potential benefits associated with different migration flows.~~
...
The use of IRS migration data, which only includes individuals from household that file taxes, introduces a potential bias, as it excludes non-filers, such as low-income populations, undocumented migrants, and others who may not be captured by tax records. This could lead to an under-representation of certain demographic groups in the migration analysis, potentially skewing the results. That said, if we observe a relationship between migration and health using the IRS data, this relationship is likely even stronger in reality as the excluded populations may experience even greater health disparities linked to migration CITE?.
...
Finally, this study only considered internal US migration, excluding migration flows from outside the United States. This focus on domestic migration limits the generalizability of our results, especially in regions where international migration plays a significant role in shaping population health dynamics. Future studies could improve upon these limitations by incorporating more comprehensive data sources that account for non-tax-filing populations and international migration flows.
